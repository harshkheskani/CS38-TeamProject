...
def populate():
    ...
    for i in sectionItems:
        add_sectionItem(i['section'], i['name'], i['description'], i['price'], i['img'])

    for h in openingHourss:
        add_openingHours(businesses[0]["name"], h["weekday_from"], h["weekday_to"], h["from_hour"], h["to_hour"])
...
def add_sectionItem(section, name, description, price, img):
...
def add_openingHours(business,weekday_from, weekday_to, from_hour, to_hour):
    new_hours = OpeningHours(business_fk=Business.objects.get(name=business))
    new_hours.weekday_from = weekday_from
    new_hours.weekday_to = weekday_to
    new_hours.from_hour = from_hour
    new_hours.to_hour = to_hour
    new_hours.save()
